...
class Preprocess:
    ...
    def __crop(image, landmarks):
        ...
        # expand bbox
        top = int(landmarks_top - 0.9 * (landmarks_bottom - landmarks_top))
        bottom = int(landmarks_bottom + 0.4 * (landmarks_bottom - landmarks_top))
        left = int(landmarks_left - 0.4 * (landmarks_right - landmarks_left))
        right = int(landmarks_right + 0.4 * (landmarks_right - landmarks_left))
